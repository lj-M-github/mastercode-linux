...
```
┌───────────────────────────────────────────────────────────────────────────────────────────┐
│  智能加固代理核心闭环 (Autonomous AI Agent Core Processing Loop)                          │
│                                                                                           │
│  ┌───────────────┐  转换  ┌───────────────┐  提取  ┌───────────────┐  生成  ┌───────────┐ │
│  │ 知识摄入模块  │───────▶│  向量知识库   │◀───────│ LLM 推理引擎  │───────▶│  执行层   │ │
│  │ (Knowledge)   │(Embed.)│  (Vector DB)  │(Retrie.)│ (The "Brain") │(Ansible)│ (Hands)   │ │
│  │               │        │               │        │               │        │           │ │
│  │- PDF 解析器   │        │- ChromaDB /   │        │- LangChain    │        │- 剧本下发 │ │
│  │- 规则分块器   │        │  Milvus       │        │- DeepSeek /   │        │- SSH 连接 │ │
│  │- 文本向量化   │        │- 规则库存储   │        │  OpenAI API   │        │- 状态捕获 │ │
│  └───────────────┘        └───────────────┘        └───────────────┘        └───────────┘ │
│                                   ▲                        ▲                      │       │
│                                   │                        │                      │       │
│                                   │ 成功模式向量化存储     │ 失败日志重写机制     ▼       │
│                                   │ (Knowledge Update)     │ (Self-Healing) ┌───────────┐ │
│                                   │                        └────────────────│ 反馈与结果│ │
│                                   │                                         │(Feedback &│ │
│                                   └─────────────────────────────────────────│  Result)  │ │
│                                                                             │- 日志清洗 │ │
